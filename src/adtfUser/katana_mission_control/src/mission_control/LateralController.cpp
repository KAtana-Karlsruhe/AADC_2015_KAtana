...
const Position2d LateralController::STRAIGHT = Position2d(1.0, 0.0);
...
float LateralController::calculateSteering(const Pose2d &vehicle_pose)
{
  ...
  if (counter % 10 == 0)
    std::cout <<"LateralController [calculateSteering] - Calculated delta " <<m_delta <<" from " <<m_trajectory.size() <<" points." <<std::endl;
...
void LateralController::updatePosAndCheckForReached(const Pose2d& vehicle_pose)
{
  ...
  if (counter % 10 == 0)
  {
    std::cout <<"Position: " <<vehicle_pose <<" Projected: " <<m_projected <<std::endl;
    std::cout <<"Reached distance: " <<(m_projected.getPosition() - m_trajectory.back().getPosition()).norm() <<std::endl;
  }
...
void LateralController::Controller(double psiArg, double thetaArg, double kappaArg, double distanceArg)
{
  ...
  static u_int32_t counter = 0;
  if (counter % 10 == 0)
    std::cout <<"LateralController [Controller] - vorsteuerung: " <<vorsteuerung <<" angleFinal: " <<angleFinal
              <<" distFinal: " <<distFinal <<" distanceArg: " <<distanceArg <<" kappa: " <<kappaArg <<std::endl;
